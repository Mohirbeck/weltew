...
async function getProducts(page = 0, category: any) {
    ...
    const res = await fetch(`${process.env.apiUrl}/products/?offset=${page * 15}&limit=15&category=${category}`)
                ...
                <div className="flex justify-center items-center space-x-1 mt-8">
                    {category.previous && (
                        <Link className="w-7 aspect-square flex justify-center items-center border border-grey rounded-md"
                            href={`/category/${router.query.id}?page=${Number(router.query.page) - 1}`}
                            scroll
                        >
                    ...
                    {category.next && (
                        <Link className="w-7 aspect-square flex justify-center items-center border border-grey rounded-md"
                            href={`/category/${router.query.id}?page=${(Number(router.query.page) + 1) || 2}`}
                            scroll
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} className="w-3 h-3 stroke-primary">
