...
export default function ProductCard({ images, name, category, id, horizontal = false, to=`/products/${id}` }) {
    function Images() {
        ...
        return (
            <div className={horizontal ? 'h-full relative group/images w-[480px]' : 'h-[220px] relative group/images'}>
                <div className="grid w-full h-full absolute top-0 left-0 z-10" style={{ gridTemplateColumns: `repeat(${images.length}, minmax(0, 1fr))` }}>
                    {
                        images.map((image) => (
                            <div key={image.id} className="h-full group/preview" data-image={image.image} onMouseEnter={changeImage} onMouseOver={changeImage}>
                            </div>
                        ))
                    }
                </div>
    ...
    return (
        <div className={horizontal ? 'bg-white border border-grey flex col-span-2 py-5 px-8' : 'bg-white border border-grey col-span-1'}>
            <Link href={to}>
                <Images />
            </Link>
